...
#define SHT10PORT GPIOE
#define SCL GPIO_Pin_6	//0824
#define SDA GPIO_Pin_5	//0824
#define noACK 0
#define ACK   1
...
/*温湿度测量标志
	...
	1:温度
	*/
extern u8 cmd_sended;
extern u8 cmd_humi;
extern u8 massuring;
extern u8 massureflag;
float humi_history[2]={0};
float temp_history[2]={0};
...
void Sht_GPIO_Config(void)
{
  ...
  GPIO_InitTypeDef  GPIO_InitStructure;
	/* 使能与 I2C1 有关的时钟 */
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOE,ENABLE);	//SHT10PORT
...
{ const float C1=-2.0468;           // for 12 Bit RH
  ...
  const float C3=-1.5955e-6;     // for 12 Bit RH
...
unsigned char s_sendcmd(unsigned char mode)
{
  unsigned char error=0;
...
unsigned char s_readresult(unsigned char *p_value, unsigned char *p_checksum)
{
  unsigned char error;
  unsigned int i;
	error=0;
...
void sht10_read1(void)
{
	...
	float temp;
	error=0;
	if(cmd_sended)//命令已发送，读取测量结果
	{
		if(cmd_humi)//读取湿度
		{	
			error+=s_readresult((unsigned char*) &humi_val.i,&checksum);//湿度
			cmd_humi=0;			
		}
		else
		{
			error+=s_readresult((unsigned char*) &temp_val.i,&checksum);//温度
			//温湿度都读过一次了，可以计算了			
			cmd_humi=1;			
			massureflag=0;//清空测量请求			
			 { 
					humi_val.f=(float)humi_val.i;                   //converts integer to float
					temp_val.f=(float)temp_val.i;                   //converts integer to float
				 ...
				 */
				 if((humi_history[0]-humi_history[1])*(humi_history[1]-humi_val.f)>0)//湿度取中间值
				 {
						humi_history[0]=humi_history[1];
					  humi_history[1]=humi_val.f;
						humi_val.f=humi_history[0];
				 }else if((humi_history[1]-humi_history[0])*(humi_history[0]-humi_val.f)>0)
				 {
					 temp=humi_history[0];
					 humi_history[0]=humi_history[1];
					 humi_history[1]=humi_val.f;
				 ...
				 if((temp_history[0]-temp_history[1])*(temp_history[1]-temp_val.f)>0)//温度取中间值
				 {
						temp_history[0]=temp_history[1];
					  temp_history[1]=temp_val.f;
						temp_val.f=temp_history[0];
				 }else if((temp_history[1]-temp_history[0])*(temp_history[0]-temp_val.f)>0)
				 {
					 temp=temp_history[0];
					 temp_history[0]=temp_history[1];
					 temp_history[1]=temp_val.f;
...
//					printf("temp:%5.1fC humi:%5.1f%%\n",temp_val.f,humi_val.f);
		...
		cmd_sended=0;//待命
		//massuring=0;//本次测量已完成	 中断里面置零
		
	}else//命令未发送
	{
		error+=s_sendcmd(cmd_humi);
		cmd_sended=1;//命令已经发送
		massuring=1;//测量中
		SDA_READ();//管脚配置为IPU输入
		EXTI->IMR|=(1<<5);//开中断等待测量完成，若寄存器的方法不行，则用库函数实现，可参考原子哥ps2实验例程
	}
